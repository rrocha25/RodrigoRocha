...
num_seg_list0 = 1:1:100;    % por exemplo, de 25 até 200
...
%         
 ...
 % --- ATRIBUIÇÃO CORRIGIDA AQUI ---
    % Atribui o vetor de medianas (num_cases elementos) à COLUNA idx_rng da matriz
    DOA_az_4_median_all(:, idx_rng) = DOA_az_5_median; 
    DOA_el_4_median_all(:, idx_rng) = DOA_el_5_median;
    DOA_az_5_median_all(:, idx_rng) = DOA_az_5_median; 
    DOA_el_5_median_all(:, idx_rng) = DOA_el_5_median;
    DOA_az_6_median_all(:, idx_rng) = DOA_az_5_median; 
    DOA_el_6_median_all(:, idx_rng) = DOA_el_5_median;
    
    DOA_az_4_mean_all(:, idx_rng) = DOA_az_5_mean; 
    DOA_el_4_mean_all(:, idx_rng) = DOA_el_5_mean;
    DOA_az_5_mean_all(:, idx_rng) = DOA_az_5_mean; 
    DOA_el_5_mean_all(:, idx_rng) = DOA_el_5_mean;
    DOA_az_6_mean_all(:, idx_rng) = DOA_az_5_mean; 
    DOA_el_6_mean_all(:, idx_rng) = DOA_el_5_mean;
end % Fim do loop idx_rng

%% 7) Agora, fora dos loops, você pode calcular RMSE e STD
% --- CÁLCULOS NECESSÁRIOS PARA O ERRO ANGULAR ---
% Vetor unitário da DOA real (já deve estar definido)
u_real_vec = [cosd(el_real)*cosd(az_real); ...
              cosd(el_real)*sind(az_real); ...
              sind(el_real)];

angular_errors_5_median = nan(num_cases, num_cases0);

for i_seg_config = 1:num_cases % 'i_seg_config' representa o índice do num_segments na lista num_seg_list
 
    current_az_estimates_for_all_rng = DOA_az_5_median_all(i_seg_config, :);
    current_el_estimates_for_all_rng = DOA_el_5_median_all(i_seg_config, :);

    u_est_vectors_for_all_rng = azel2unitvec(current_az_estimates_for_all_rng, current_el_estimates_for_all_rng);
    angular_errors_5_median(i_seg_config, :) = ang_err_deg(u_real_vec, u_est_vectors_for_all_rng);
end

% RMSE: Raiz quadrada da média dos erros quadráticos ao longo das realizações (dimensão 2)
rmse_per_num_segments_5_median = sqrt(nanmean(angular_errors_5_median.^2, 2));

% STD: Desvio padrão dos erros ao longo das realizações (dimensão 2)
std_per_num_segments_5_median = nanstd(angular_errors_5_median, 0, 2); % 0 para normalização padrão, 2 para dimensão

fprintf('\n--- RMSE e STD para 5 Pares(Mediana) Tamanho do segmento: %.1f segundos  ---\n', 1000*dur_s);
for i_seg_config = 1:num_cases
    current_num_segments = num_seg_list(i_seg_config);
    fprintf('Num Segments = %4d: RMSE = %.2f deg, STD = %.2f deg\n', ...
            current_num_segments, ...
            rmse_per_num_segments_5_median(i_seg_config), ...
            std_per_num_segments_5_median(i_seg_config));
end

%% =========================================================================
% PLOTAGEM AZIMUTE vs ELEVAÇÃO (para todas as 100 rodadas)
% =========================================================================

% Cores para os gradientes (ajuste conforme preferir)
base_color_6 = [0.0 0.4470 0.7410]; % Azul para 6 pares
base_color_5 = [0.8500 0.3250 0.0980]; % Laranja para 5 pares
...
for k = 1:num_color_steps
    % Cria um gradiente de cores, do mais claro ao mais escuro
    % O fator de escurecimento pode ser ajustado (e.g., de 0.3 a 1.0)
    darkening_factor = 0.3 + (k-1) * (0.7 / (num_color_steps-1));
    colors_gradient_6(k,:) = base_color_6 * darkening_factor;
    colors_gradient_5(k,:) = base_color_5 * darkening_factor;
end

figure('Position', [100 100 fig_width fig_height], 'Color', 'w');
hold on; grid on; box on;

% Plotar a DOA real
h_real = plot(az_real, el_real, 'p', 'MarkerSize', 40, 'MarkerFaceColor', 'y', 'MarkerEdgeColor', 'k', 'LineWidth', 3, 'DisplayName', 'Real DOA');

% Handles para a legenda (inicializados como vazios)
h_med_6_legend = [];
h_med_5_legend = [];
h_mean_6_legend = [];
h_mean_5_legend = [];

% Loop para plotar as medianas e médias de CADA RODADA para cada configuração de num_segments
for idx_case = 1:num_cases % Itera sobre as configurações de num_segments
    current_color_6 = colors_gradient_6(idx_case,:);
    current_color_5 = colors_gradient_5(idx_case,:);

    % Plotar Mediana para 6 Pares (todos os 100 resultados de rodadas)
    if ~all(isnan(DOA_az_6_median_all(idx_case, :))) && ~all(isnan(DOA_el_6_median_all(idx_case, :)))
        h_plot = plot(DOA_az_6_median_all(idx_case, :), DOA_el_6_median_all(idx_case, :), '^', 'MarkerSize', 10, 'MarkerFaceColor', current_color_6, 'MarkerEdgeColor', 'k', 'LineWidth', 0.5, 'HandleVisibility', 'off');
        if isempty(h_med_6_legend)
            h_med_6_legend = h_plot; % Guarda o handle da primeira plotagem para a legenda
        end
    end

    % Plotar Mediana para 5 Pares (todos os 100 resultados de rodadas)
    if ~all(isnan(DOA_az_5_median_all(idx_case, :))) && ~all(isnan(DOA_el_5_median_all(idx_case, :)))
        h_plot = plot(DOA_az_5_median_all(idx_case, :), DOA_el_5_median_all(idx_case, :), '^', 'MarkerSize', 10, 'MarkerFaceColor', current_color_5, 'MarkerEdgeColor', 'k', 'LineWidth', 0.5, 'HandleVisibility', 'off');
        if isempty(h_med_5_legend)
            h_med_5_legend = h_plot; % Guarda o handle da primeira plotagem para a legenda
        end
    end

    % Plotar Média para 6 Pares (todos os 100 resultados de rodadas)
    if ~all(isnan(DOA_az_6_mean_all(idx_case, :))) && ~all(isnan(DOA_el_6_mean_all(idx_case, :)))
        h_plot = plot(DOA_az_6_mean_all(idx_case, :), DOA_el_6_mean_all(idx_case, :), 'd', 'MarkerSize', 10, 'MarkerFaceColor', current_color_6, 'MarkerEdgeColor', 'k', 'LineWidth', 0.5, 'HandleVisibility', 'off');
        if isempty(h_mean_6_legend)
            h_mean_6_legend = h_plot; % Guarda o handle da primeira plotagem para a legenda
        end
    end

    % Plotar Média para 5 Pares (todos os 100 resultados de rodadas)
    if ~all(isnan(DOA_az_5_mean_all(idx_case, :))) && ~all(isnan(DOA_el_5_mean_all(idx_case, :)))
        h_plot = plot(DOA_az_5_mean_all(idx_case, :), DOA_el_5_mean_all(idx_case, :), 'd', 'MarkerSize', 10, 'MarkerFaceColor', current_color_5, 'MarkerEdgeColor', 'k', 'LineWidth', 0.5, 'HandleVisibility', 'off');
        if isempty(h_mean_5_legend)
            h_mean_5_legend = h_plot; % Guarda o handle da primeira plotagem para a legenda
        end
    end
end
...
figure('Position', [100 100 1000 1500], 'Color', 'w');
hold on;
grid on;
box on;
...
set(gca, 'FontSize', 30);
set(gca, 'Position', [0.12 0.15 0.84 0.78]);
...
print(fullfile(outputDir, 'doa_az_el_plot_6_5_pairs_segments_color.eps'));
%}
